...
export const RelayDeviceTokenPollCode = Schema.Literals([
  "authorization_pending",
  "slow_down",
  "access_denied",
  "expired_token",
]);
export type RelayDeviceTokenPollCode = typeof RelayDeviceTokenPollCode.Type;

export class RelayDeviceTokenPollError extends Schema.TaggedErrorClass<RelayDeviceTokenPollError>()(
  ...
  override get message(): string {
    return `Relay device authorization is not redeemable: ${this.code}`;
  }
}

export class RelayDeviceAuthorizationNotFoundError extends Schema.TaggedErrorClass<RelayDeviceAuthorizationNotFoundError>()(
  "RelayDeviceAuthorizationNotFoundError",
  {
    code: Schema.Literal("device_authorization_not_found"),
    traceId: TrimmedNonEmptyString,
  },
  { httpApiStatus: 404 },
) {
  override get message(): string {
    return "Relay device authorization was not found, already handled, or expired.";
  }
}
...
export const RelayDeviceAuthorizationGrantType =
  "urn:ietf:params:oauth:grant-type:device_code" as const;

// Bounds mirror the relay_device_authorizations column widths so overlong
// input fails request validation instead of surfacing as a persistence error.
const RelayDeviceClientId = TrimmedNonEmptyString.check(Schema.isMaxLength(191));
const RelayDeviceScope = TrimmedNonEmptyString.check(Schema.isMaxLength(256));
const RelayDeviceCodeChallenge = TrimmedNonEmptyString.check(Schema.isMaxLength(128));

export const RelayDeviceAuthorizationRequest = Schema.Struct({
  client_id: RelayDeviceClientId.annotate({
    description: "Clerk OAuth client id of the CLI requesting device authorization.",
  }),
  scope: RelayDeviceScope.annotate({
    description: "Space-separated OAuth scopes requested from Clerk.",
  }),
  code_challenge: RelayDeviceCodeChallenge.annotate({
    description: "PKCE code challenge generated by the device.",
  }),
  code_challenge_method: Schema.Literal("S256"),
  device_name: Schema.optional(
    TrimmedNonEmptyString.check(Schema.isMaxLength(191)).annotate({
      description: "Human-readable device name shown on the approval page.",
    }),
  ),
  device_platform: Schema.optional(
    TrimmedNonEmptyString.check(Schema.isMaxLength(128)).annotate({
      description: "Platform descriptor shown on the approval page.",
    }),
  ),
  client_version: Schema.optional(
    TrimmedNonEmptyString.check(Schema.isMaxLength(64)).annotate({
      description: "CLI version shown on the approval page.",
    }),
  ),
})
  .annotate({ description: "Starts an OAuth device authorization for a browserless client." })
  .pipe(HttpApiSchema.asFormUrlEncoded());
export type RelayDeviceAuthorizationRequest = typeof RelayDeviceAuthorizationRequest.Type;

export const RelayDeviceAuthorizationResponse = Schema.Struct({
  device_code: TrimmedNonEmptyString,
  user_code: TrimmedNonEmptyString,
  verification_uri: TrimmedNonEmptyString,
  verification_uri_complete: TrimmedNonEmptyString,
  expires_in: Schema.Int.check(Schema.isGreaterThan(0)),
  interval: Schema.Int.check(Schema.isGreaterThan(0)),
});
export type RelayDeviceAuthorizationResponse = typeof RelayDeviceAuthorizationResponse.Type;

export const RelayDeviceTokenRequest = Schema.Struct({
  grant_type: Schema.Literal(RelayDeviceAuthorizationGrantType),
  device_code: TrimmedNonEmptyString.check(Schema.isMaxLength(128)),
  client_id: RelayDeviceClientId,
})
  .annotate({ description: "Polls a pending device authorization for its granted code." })
  .pipe(HttpApiSchema.asFormUrlEncoded());
export type RelayDeviceTokenRequest = typeof RelayDeviceTokenRequest.Type;

export const RelayDeviceTokenResponse = Schema.Struct({
  authorization_code: TrimmedNonEmptyString.annotate({
    description:
      "Clerk authorization code bound to the device's PKCE challenge. The device exchanges it directly at the Clerk token endpoint.",
  }),
  redirect_uri: TrimmedNonEmptyString.annotate({
    description: "Redirect URI used in the brokered authorization; echo it in the token exchange.",
  }),
});
export type RelayDeviceTokenResponse = typeof RelayDeviceTokenResponse.Type;

export const RelayDeviceAuthorizationStatus = Schema.Literals(["pending", "approved", "denied"]);
export type RelayDeviceAuthorizationStatus = typeof RelayDeviceAuthorizationStatus.Type;

export const RelayDeviceAuthorizationDetails = Schema.Struct({
  userCode: TrimmedNonEmptyString,
  status: RelayDeviceAuthorizationStatus,
  scope: TrimmedNonEmptyString,
  ...
  requestIp: Schema.NullOr(Schema.String),
  requestLocation: Schema.NullOr(Schema.String),
  requestedAt: TrimmedNonEmptyString,
  expiresAt: TrimmedNonEmptyString,
});
export type RelayDeviceAuthorizationDetails = typeof RelayDeviceAuthorizationDetails.Type;

export const RelayDeviceAuthorizationParams = Schema.Struct({
  userCode: TrimmedNonEmptyString.check(Schema.isMaxLength(32)),
});
export type RelayDeviceAuthorizationParams = typeof RelayDeviceAuthorizationParams.Type;

export const RelayDeviceApprovalResponse = Schema.Struct({
  authorizationUrl: TrimmedNonEmptyString.annotate({
    description:
      "Clerk authorize URL the approving browser must visit to mint the device's authorization code.",
  }),
});
export type RelayDeviceApprovalResponse = typeof RelayDeviceApprovalResponse.Type;

export const RelayDeviceCompletionRequest = Schema.Struct({
  state: TrimmedNonEmptyString.check(Schema.isMaxLength(64)).annotate({
    description: "Opaque state returned by the approval step and echoed by Clerk.",
  }),
  code: TrimmedNonEmptyString.check(Schema.isMaxLength(512)).annotate({
    description: "Authorization code issued by Clerk for the approved device.",
  }),
});
export type RelayDeviceCompletionRequest = typeof RelayDeviceCompletionRequest.Type;
...
const RelayDeviceTokenErrors = [RelayDeviceTokenPollError, RelayInternalError] as const;
const RelayDeviceApprovalErrors = [
  RelayAuthInvalidError,
  RelayDeviceAuthorizationNotFoundError,
  RelayInternalError,
] as const;

export const RelayDeviceGroup = HttpApiGroup.make("device")
  .add(
    HttpApiEndpoint.post("authorizeDevice", "/v1/device/authorization", {
      payload: RelayDeviceAuthorizationRequest,
      success: RelayDeviceAuthorizationResponse,
      error: RelayInternalError,
    }).annotate(OpenApi.Summary, "Start a device authorization"),
    HttpApiEndpoint.post("redeemDeviceCode", "/v1/device/token", {
      payload: RelayDeviceTokenRequest,
      success: RelayDeviceTokenResponse,
      error: RelayDeviceTokenErrors,
    }).annotate(OpenApi.Summary, "Poll a device authorization for its granted code"),
  )
  .annotate(
    OpenApi.Description,
    "OAuth device authorization for browserless CLIs, brokered through the Clerk authorization-code flow.",
  );

export const RelayDeviceApprovalGroup = HttpApiGroup.make("deviceApproval")
  .add(
    HttpApiEndpoint.get("getDeviceAuthorization", "/v1/client/device-authorizations/:userCode", {
      headers: RelayBearerRequestHeaders,
      params: RelayDeviceAuthorizationParams,
      success: RelayDeviceAuthorizationDetails,
      error: RelayDeviceApprovalErrors,
    }).annotate(OpenApi.Summary, "Look up a pending device authorization by user code"),
    HttpApiEndpoint.post(
      "approveDeviceAuthorization",
      "/v1/client/device-authorizations/:userCode/approve",
      {
        headers: RelayBearerRequestHeaders,
        params: RelayDeviceAuthorizationParams,
        success: RelayDeviceApprovalResponse,
        error: RelayDeviceApprovalErrors,
      },
    ).annotate(OpenApi.Summary, "Approve a device authorization"),
    HttpApiEndpoint.post(
      "completeDeviceAuthorization",
      "/v1/client/device-authorizations/complete",
      {
        headers: RelayBearerRequestHeaders,
        payload: RelayDeviceCompletionRequest,
        success: RelayOkResponse,
        error: RelayDeviceApprovalErrors,
      },
    ).annotate(OpenApi.Summary, "Attach the Clerk authorization code to an approved device"),
    HttpApiEndpoint.post(
      "denyDeviceAuthorization",
      "/v1/client/device-authorizations/:userCode/deny",
      {
        headers: RelayBearerRequestHeaders,
        params: RelayDeviceAuthorizationParams,
        success: RelayOkResponse,
    ...
    ).annotate(OpenApi.Summary, "Deny a device authorization"),
  )
  .annotate(OpenApi.Description, "Signed-in approval surface for pending device authorizations.")
  .middleware(RelayClientAuth);
