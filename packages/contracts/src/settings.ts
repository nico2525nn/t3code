...
export const OpenCode2Settings = makeProviderSettingsSchema(
  {
    // Off by default (like OpenCode/Cursor/Grok): the V2 preview binding is
    // not stable enough to probe on every install. Users opt in from Settings.
    enabled: Schema.Boolean.pipe(
      Schema.withDecodingDefault(Effect.succeed(false)),
      Schema.annotateKey({ providerSettingsForm: { hidden: true } }),
    ),
    binaryPath: makeBinaryPathSetting("opencode2").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description: "Path to the OpenCode 2 (preview) binary.",
        providerSettingsForm: {
