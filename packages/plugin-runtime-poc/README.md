...
| ----------------------- | -----------: | --------------------------------------------------- | ------------------------------------------------------------------ |
| `effectScopeRuntime.ts` |          365 | none, effect is already used by t3                  | best base for production                                           |
| `cordisRuntime.ts`      |          428 | cordis rc.8 and about 300 kib of installed packages | works, but adds a second lifecycle runtime and the largest adapter |
| `pureRuntime.ts`        |          343 | none                                                | simplest planner, but manual finalizers are too easy to misuse     |
...
all variants run the same contract suite. it checks:
...
| --------------- | -----------: | ------------------: | ----------: |
| effect scope    |      5.55 ms |             1.05 ms |     4.46 ms |
| cordis          |     97.30 ms |             1.20 ms |   303.93 ms |
| pure reconciler |      0.94 ms |             0.55 ms |     0.57 ms |
