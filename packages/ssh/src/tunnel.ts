...
function resolveDurationInput(
  duration: Duration.Input | undefined,
  durationMs: number | undefined,
  fallback: Duration.Duration,
): Duration.Duration {
  return Duration.fromInputUnsafe(duration ?? durationMs ?? fallback);
}
...
export const waitForHttpReady = Effect.fn("ssh/tunnel.waitForHttpReady")(function* (input: {
  ...
  /** @deprecated Prefer `interval` with an Effect Duration input. */
  readonly intervalMs?: number;
  /** @deprecated Prefer `probeTimeout` with an Effect Duration input. */
  readonly probeTimeoutMs?: number;
  readonly path?: string;
}): Effect.fn.Return<void, SshReadinessError, HttpClient.HttpClient> {
  const timeout = resolveDurationInput(input.timeout, input.timeoutMs, DEFAULT_HTTP_READY_TIMEOUT);
  const interval = resolveDurationInput(
    input.interval,
    input.intervalMs,
    DEFAULT_HTTP_READY_INTERVAL,
  );
  const probeTimeout = resolveDurationInput(
    input.probeTimeout,
    input.probeTimeoutMs,
    SSH_READY_PROBE_TIMEOUT,
  );
