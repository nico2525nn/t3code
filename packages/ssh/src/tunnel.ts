...
export const waitForHttpReady = Effect.fn("ssh/tunnel.waitForHttpReady")(function* (input: {
  ...
  const timeout = normalizePositiveDuration(input.timeout, Duration.seconds(30));
  const interval = normalizePositiveDuration(input.interval, Duration.millis(100));
  const probeTimeout = normalizePositiveDuration(input.probeTimeout, SSH_READY_PROBE_TIMEOUT);
