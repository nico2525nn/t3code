...
const make = Effect.gen(function* () {
  ...
  const getActiveAssistantMessageIdForTurn = (
    threadId: ThreadId,
    turnId: TurnId,
    eventOrRole?: ProviderRuntimeEvent | MessageStreamRole,
    role: MessageStreamRole = "assistant",
  ) =>
    getAssistantSegmentStateForTurn(
      threadId,
      turnId,
      typeof eventOrRole === "string" ? eventOrRole : role,
    ).pipe(
      Effect.map((state) =>
        Option.flatMap(state, (entry) => {
          const nativeItemKey =
            eventOrRole !== undefined &&
            typeof eventOrRole !== "string" &&
            String(eventOrRole.provider) === "codex" &&
            eventOrRole.itemId !== undefined
              ? assistantSegmentBaseKeyFromEvent(eventOrRole)
              : undefined;
          if (nativeItemKey !== undefined && entry.baseKey !== nativeItemKey) {
            return Option.none();
          }
          return entry.activeMessageId
    ...
    Effect.gen(function* () {
      ...
      if (assistantCompletion) {
        ...
        const activeAssistantMessageId = turnId
          ? yield* getActiveAssistantMessageIdForTurn(thread.id, turnId, event)
          : Option.none<MessageId>();
        ...
        const shouldSkipRedundantCompletion =
          ...
          !existingAssistantMessage.isStreaming &&
