  ...
  return Effect.gen(function* () {
    ...
    const dependencies = Layer.mergeAll(
      ...
      ServerSettingsService.layerTest({
        threadAutoSettleOnMerge: input.autoSettleOnMerge ?? true,
      }),
...
it.effect("keeps merged pinned threads active when merge settlement is disabled", () =>
  Effect.scoped(
    Effect.gen(function* () {
      const thread = makeThread({
        id: "pinned-merged-disabled",
        branch: "feature/merged-disabled",
        cwd: "/repo/merged-disabled",
        pinned: true,
      });
      const pollCalls = yield* Ref.make(0);
      const { reactor, dispatched } = yield* makeHarness({
      ...
      expect(yield* Ref.get(pollCalls)).toBe(0);
