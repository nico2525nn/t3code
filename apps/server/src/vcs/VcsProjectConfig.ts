...
const decodeProjectVcsConfig = Schema.decodeOption(Schema.fromJsonString(ProjectVcsConfig));
...
export const make = Effect.fn("makeVcsProjectConfig")(function* () {
  ...
  const readConfiguredKind = Effect.fn("VcsProjectConfig.readConfiguredKind")(function* (
    ...
    return yield* Option.match(parseConfig(raw.value), {
      onSome: (parsed) => Effect.succeed(configuredKind(parsed)),
      onNone: () =>
        Effect.logWarning("invalid VCS project config", {
          configPath,
