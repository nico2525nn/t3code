...
import { ChatAttachment, OrchestrationMessageNotification } from "@t3tools/contracts";
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextNotificationJson =
        row.notification !== undefined ? JSON.stringify(row.notification) : null;
      return sql`
        ...
        DO UPDATE SET
          ...
          notification_json = COALESCE(
            excluded.notification_json,
            projection_thread_messages.notification_json
          ),
