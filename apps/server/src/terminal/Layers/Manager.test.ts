...
it.layer(
  ...
  it.effect("restarts a running session when open is called with a different cwd", () =>
    Effect.gen(function* () {
      ...
      const originalCwd = pathService.join(baseDir, "original");
      const differentCwd = pathService.join(baseDir, "different");
      ...
      yield* waitFor(pathExists(historyPath));
      ...
      yield* waitFor(Effect.map(readFileString(historyPath), (text) => text === ""));
    }),
  );
