...
it.layer(NodeServices.layer)("CodexAppServerManager", (it) => {
  it.effect("reconnects catalog reads after the shared daemon exits", () =>
    Effect.gen(function* () {
      const markerPath = NodePath.join(
        NodeOS.tmpdir(),
        `t3-codex-manager-catalog-reconnect-${process.pid}-${DateTime.toEpochMillis(DateTime.nowUnsafe())}.log`,
      );
      ...
      const manager = yield* makeCodexAppServerManager({
        instanceId: ProviderInstanceId.make("codex-manager-catalog-reconnect-test"),
        binaryPath: process.execPath,
        cwd: fixtureDirectory,
        environment: {
          ...process.env,
          T3_CODEX_MANAGER_TEST_MARKER: markerPath,
          T3_CODEX_MANAGER_TEST_CATALOG_RESTART_ONCE: "1",
        },
      });
      yield* Effect.addFinalizer(() => manager.close);

      const threads = yield* manager
        .listThreads({ archived: false, limit: 1 })
        .pipe(Effect.timeout("5 seconds"));
      expect(threads.map((thread) => thread.id)).toEqual(["catalog-thread-1", "catalog-thread-2"]);
      expect(threads[0]?.updatedAt).toBe(2);
      expect(NodeFS.readFileSync(markerPath, "utf8")).toContain("catalog-restart-injected\n");
      expect(NodeFS.readFileSync(markerPath, "utf8").split("started\n").length - 1).toBe(2);
  ...
  it.effect("shares one daemon with independent App Server connections", () =>
    Effect.gen(function* () {
      const markerPath = NodePath.join(
        NodeOS.tmpdir(),
      ...
      const first = yield* manager.openSession().pipe(Effect.timeout("5 seconds"));
      const second = yield* manager.openSession().pipe(Effect.timeout("5 seconds"));
      ...
      const secondUnknownRequestResponses = yield* Queue.unbounded<unknown>();
      ...
      yield* second.client.handleUnknownServerNotification((method, payload) => {
        ...
        if (method === "test/unknown-response-seen") {
          return Queue.offer(secondUnknownRequestResponses, payload).pipe(Effect.asVoid);
        }
        return Effect.void;
      });
      ...
      const unknownResponse = yield* Queue.take(secondUnknownRequestResponses).pipe(
        Effect.timeout("5 seconds"),
      );
      ...
      assert.equal((secondGlobal as { value: string }).value, `global for ${secondThreadId}`);
      ...
      assert.deepEqual((unknownResponse as { result: unknown }).result, { owner: "second" });
      ...
      const runtime = yield* makeCodexSessionRuntime({
        ...
        appServerSession: runtimeSession,
      ...
      const reconnected = yield* manager.openSession().pipe(Effect.timeout("5 seconds"));
