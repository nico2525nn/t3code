...
function enforceRetention(input: {
  ...
  try {
    entries = fs.readdirSync(input.directory, { withFileTypes: true });
  } catch (cause) {
...
function drainPending(input: {
  ...
  for (const [threadSegment, records] of recordsBySegment) {
    ...
    const filePath = path.join(input.directory, `${threadSegment}.log`);
    ...
    ? enforceRetention({
        ...
        activeFilePaths: new Set(
          Array.from(sinks.keys(), (threadSegment) =>
            path.join(input.directory, `${threadSegment}.log`),
          ),
        ),
        now: input.now,
      })
