...
/** The sidebar list holds queued offline tasks, flat thread rows, and a
    settled "Show more" pager. */
...
function ThreadNavigationSidebarPane(
  ...
  // Same model as the compact Home list (HomeScreen.tsx): flat
  // creation-order card block + settled recency tail. PR states stream in
  // per-row; merged/closed PRs auto-settle their thread on the next
  // partition.
  ...
  const listItems = useMemo<readonly SidebarListItem[]>(() => {
    ...
    // compact Home list): they are not thread shells, so the item builder
    // never sees them, but they must stay visible and deletable while their
    // environment is offline. Same environment scope and search filter as
    // the list.
  ...
  // The list lays out in fixed creation order, so environment and project are
  // the only filters that can be customized.
