...
function getBinaryLaunchSpec(
  ...
  // Try current platform key (e.g. "darwin-arm64", "linux-x64")
          ...
          <div className="mx-auto flex w-full max-w-2xl flex-col gap-6">
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
                              {filteredRegistryAgents.map((entry) => {
                                ...
                                const isUpToDate =
                                  isInstalled &&
                                  installedAgent?.importedVersion === entry.agent.version;
                          ...
                          <div className="space-y-4">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if (
                                  manualAcpName.trim() &&
                                  manualAcpCommand.trim() &&
                                  !/\s/.test(manualAcpCommand.trim()) &&
                                  !settings.providers.acp.agentServers.some(
                                    (agent) => agent.id === slugifyAcpAgentId(manualAcpName.trim()),
                                  )
                                ) {
            ...
            >
              {registeredAcpAgents.map((agent) => {
                const status = acpAgentStatuses.find(
                  (candidate) => candidate.agentServerId === agent.id,
                );
                return (
                  <div
                    key={agent.id}
                        ...
                        <div className="min-w-0 flex-1 space-y-1">
                          <div className="flex min-h-5 items-center gap-1.5">
                            <span
                              className={cn(
                                "size-2 shrink-0 rounded-full",
                                agent.enabled ? "bg-success" : "bg-amber-400",
                              )}
                            />
              ...
              {registeredAcpAgents.length === 0 ? (
                ...
                >
                  <p className="text-xs text-muted-foreground">
                    No ACP agents registered. Click "Add agent" to browse the registry or configure
                    one manually.
                  </p>
                </div>
              ) : null}
