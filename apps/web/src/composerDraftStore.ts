...
import { isComposerImageMimeType } from "./lib/composerAttachments";
import {
...
/**
 * A non-image file staged in the composer. Only Hermes threads can produce
 * these — every other provider's intake stays image-only — but the draft
 * state holds them in the same `images` array so attachments share one
 * lifecycle (dedupe, persist, hydrate, stash, clear) instead of two.
 */
...
export function hydrateImagesFromPersisted(
  ...
  return attachments.flatMap((attachment) => {
    ...
    // The persisted shape carries no variant tag, so it is re-derived from
    // the mime exactly as intake classifies it. Persisting the tag instead
    // would need a storage migration for no behavioural gain.
    return [
      {
        ...(isComposerImageMimeType(attachment.mimeType)
          ? { type: "image" as const }
          : { type: "file" as const }),
        id: attachment.id,
