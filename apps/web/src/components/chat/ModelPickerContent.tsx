...
const ModelPickerSearchInput = memo(function ModelPickerSearchInput(props: {
  ...
  onNormalizedSearchQueryChange: (query: string) => void;
  onRequestClose?: () => void;
  onSelectHighlightedModel: () => boolean;
}) {
  const { inputRef, onNormalizedSearchQueryChange, onRequestClose, onSelectHighlightedModel } =
    props;
  const [rawSearchQuery, setRawSearchQuery] = useState("");
  const publishedSearchQueryRef = useRef(normalizeModelPickerSearchQuery(rawSearchQuery));
  ...
  const handleSearchChange = useCallback(
    (event: ChangeEvent<HTMLInputElement>) => {
      ...
      const nextSearchQuery = normalizeModelPickerSearchQuery(nextRawSearchQuery);
      if (nextSearchQuery === publishedSearchQueryRef.current) {
        return;
      }
      publishedSearchQueryRef.current = nextSearchQuery;
      onNormalizedSearchQueryChange(nextSearchQuery);
    },
    [onNormalizedSearchQueryChange],
  );
  ...
  const handleSearchKeyDown = useCallback(
    (event: KeyboardEvent<HTMLInputElement>) => {
      ...
      if (event.key === "Enter" && onSelectHighlightedModel()) {
    ...
    [onRequestClose, onSelectHighlightedModel],
  );
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  const selectHighlightedModel = useCallback((): boolean => {
